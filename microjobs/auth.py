...
def verify_email():
    ...
    if user.get("verification_expires") and datetime.utcnow() > datetime.fromisoformat(user["verification_expires"]):
...
def resend_verification():
    ...
    verify_url = url_for("auth.verify_email", token=token_raw, _external=True)
    api.send_email(to=email, subject="Verify your JobLink account",
                   html=render_template("emails/verify.html", name=user["name"], verify_url=verify_url))
...
def login():
    ...
    if user.get("lockout_until"):
        ...
        if datetime.utcnow() < datetime.fromisoformat(user["lockout_until"]):
            flash("Account temporarily locked due to multiple failed attempts.", "warning")
            return render_template("login.html")
    ...
    if not api.verify_password(user, pw):
        ...
        api.update_user(user["id"], {"failed_attempts": failed, "lockout_until": lock_until})
        log_event(user["id"], "LOGIN_FAILED", {"ip": ip, "ua": ua})
        flash("Invalid credentials.", "danger")
        return render_template("login.html")

    # success
    api.update_user(user["id"], {"failed_attempts": 0, "lockout_until": None, "last_login_at": datetime.utcnow().isoformat(), "last_login_ip": ip})
